...
Do not pretend unknown inputs are known. Do not invent suggestion data sources, analytics availability, legal-sensitive personalization rules, file paths, services, feature flags, or owners. Mark unknowns as `TBD`, `likely area`, `needs repo inspection`, or `decision required`.
...
Split schema, backfill, app read/write changes, tests, rollout, rollback, and cleanup into separate packets when their context differs. Schema usually `blocks` backfill and app-code work. Backfill may `inform` rollout validation. Mark missing feature-flag details as `decision`. Treat fallback removal as a later-release packet unless the source plan clearly says otherwise.
...
**Validation for:** Use `P004`, or `None`.
...
- clear about status and directional typed dependencies
